...
def dTs_dt_surface(n,
    ...
    """
    Retourne dT_s/dt sur le CV de surface.
    Forme discrète de l'équation (29) avec rho * c_v explicites.
    """
...
dTbdt          = np.zeros(Nx)          # Stocke les variations de Tb entre t et t+dt pour calculs de variationd e couche limite
...
for it in range(num_iterations):
    ...
    print("Tempéraure de surface" ,Ts)
    print("Tempéraure de bulk",temp_bulk[7])
    print("Tempéraure de couche limite", temp_moy_cl[7])
    q_in_s = k_propane_liq(Ts)*(Tw_s-Ts)
    Tw_s= Tw_s+ dt*dTw_dt_surface(qext, q_in_s, delta_w, c_w, lambda_w,Tw[-1], rho_w, Tw_s, Tw_vap, Hs)
    ...
    Ts= Ts+dt*dTs_dt_surface(Nx-1,m_point_B,enthalpie_B,m_point_CL, enthalpie_moy_CL,R, Hs,q_in_s,k_lint,Tint, Ts,delta, lambda_liquide, temp_bulk,dx,DHsDT,rho_liquide,cv_liquide)
    ...
    dTv_dt_i = dTv_dt(k_propane_vap(Tv), H_vap*R, Tw_vap, Tv, k_vint, Tint, cp_vapeur, H_vap*R*1 , rho_vapeur,R)
    Tv += dTv_dt_i * dt
