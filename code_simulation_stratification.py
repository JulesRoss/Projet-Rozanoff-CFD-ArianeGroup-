...
def dTB_dt_bulk(i, lambdaB, R, rho, cv,
    ...
    """
    dTB/dt (bulk) avec conduction axiale et advection (forme 1D sur un CV annulaire).
    Écriture compacte :
      dTB/dt = [ ṁ_B h_B - ṁ_B- h_B- - ṁ_y h_y + π (R-δ)^2 λB (∂TB/∂x - ∂TB-/∂x) ]
               / [ π ρ c_v (R-δ)^2 dx ]
    Ici on approxime (∂TB/∂x - ∂TB-/∂x) ≈ TB'(i) - TB'(i-1), fournis via d_dx_central.
    """
    area_i = np.pi * (R - delta[i])**2
    if (i == Nx-1):
        dTBdx_i = (TS - TB[i]) /dx
    else:
        dTBdx_i   = d_dx_central(TB, i, dx)
    dTBdx_im1 = d_dx_central(TB, max(i-1, 0), dx)

    num = (m_dot_B[i] * h_B[i]
           - m_dot_B[i-1] * h_B[i-1]
           - m_dot_y[i] * h_y[i]
           + area_i * lambdaB * (dTBdx_i - dTBdx_im1))
    den = area_i * rho * cv * dx
...
def Rayleigh_v_int(g, alpha_v, Tv, Tint, D, nu_v, a_v):
    return (g * alpha_v * (Tv - Tint) * D**3) / (nu_v * a_v)
...
def dTw_dt_int(qext_int, qin_int, delta_w, c_w, lambda_w,
               Tw_liq_array, i_int, rho_w, Tw_int, Tw_vap, H_int):
    """
    dTw_int/dt (paroi à l’interface). On référence la cellule i_int côté liquide.
    """
    return ((qext_int - qin_int) / clamp_den(delta_w * rho_w * c_w)
            + lambda_w * (Tw_int - Tw_liq_array[i_int] - (Tw_vap - Tw_int))
            / clamp_den(H_int**2 * rho_w * c_w))
...
temp_bulk      = np.zeros(Nx)          # TB
temp_moy_cl    = np.zeros(Nx)          # T_tilde (T̃)
...
Tw             = np.zeros(Nx)          # température mur côté liquide
eta            = np.zeros(Nx)          # η = (T̃-TB)/(Tw-TB)

# --- grandeurs dôme/iface/paroi (valeurs de départ) ---
Tw_vap = 2.0   # K
Tw_int = 2.0   # K
Tv     = 2.0   # K
Pv     = 2.0   # bar (attention aux unités si tu les utilises dans dP/dt)
q_ext  = 20.0  # W/m^2

# hauteur du volume de contrôle vertical (CV)
Hcv = dx
...
for it in range(num_iterations):
    ...
    Ra_vint = Rayleigh_v_int(g, alpha_v, Tv, Tint, D, nu_v, a_v)
    Ra_lint = Rayleigh_l_int(g, alpha_l, Ts, Tint, D, nu_l, a_l)
    k_vint = k_v_int(Ra_vint, D, lambda_v)
    k_lint = k_l_int(Ra_lint, D, lambda_l)
    m_dot_phase(R, gamma, Tv, Ts, Tint, k_v_int_val, k_l_int_val)
    ...
    for i in range (Nx-1,-1,-1):
        ...
        temp_bulk[i] = temp_bulk[i] + dTB_dt_bulk(i, lambdaB, R, rho_liquide, cv_liquide,temp_bulk, temp_sur, delta, dx, m_point_B, m_point_y, h_y, h_B)
