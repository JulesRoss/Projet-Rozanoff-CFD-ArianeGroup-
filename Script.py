...
flux_chaleur = lambda k_pl_10_5_9, temp_paroi,temp_bulk ,Rax  : k_pl_10_5_9*(temp_paroi-temp_bulk) if 10**9>Rax or Rax>10**5 else k_pl_10_9_13  ## 16
...
eta_moy_d = lambda temp_moy_cl, temp_paroi, temp_bulk, eta_moy, delta, i : 0 if ((delta[i]- delta[i-1]/(hauteur/Nx)) >=0) else (((temp_moy_cl[i] - temp_moy_cl[i-1])/(h) - eta_moy*((temp_paroi[i]-temp_paroi[i-1])/h) + (1-eta_moy)*(temp_bulk[i]-temp_bulk[i-1])/h)/(temp_paroi[i]-temp_bulk[i]))  ## 20
...
d_Ts_dt = lambda                     : (m_point_B[n]*enthalpie_B[n]- m_point[n]*enthalpie[n]+2*np.pi*R*H_S*qin_s+np.pi*R**2*kint_liq*(temp_int-temp_S)- np.pi*(R_delta[i])**2*lambda_B*(temp_B[n]-temp_B[n-1])/(h/Nx))/(np.pi*R**2*H_S)-(temp_S-temp_int)*d_H_S/H_S
